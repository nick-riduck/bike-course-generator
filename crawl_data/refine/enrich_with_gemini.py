#!/usr/bin/env python3
"""
Step 2: Gemini 3.1 Pro + Google Search grounding으로 POI 보강.
description, waypoint_type[], nearby_landmarks 추출.

안정성 개선:
- API 호출 timeout (60초)
- 실패 시 3회 재시도 (exponential backoff)
- 매 건마다 progress 저장 (JSONL append)
- 완료된 POI 자동 스킵 (resume)

Usage:
    python enrich_with_gemini.py                    # 전체 실행 (자동 resume)
    python enrich_with_gemini.py --limit 10         # 10개만 테스트
    python enrich_with_gemini.py --reset             # 처음부터 다시
    python enrich_with_gemini.py --workers 3         # 동시 3개 처리
...
def main():
    ...
    errors = 0
    pbar = tqdm(total=len(todo_indices), desc="POI 보강", unit="poi",
                initial=0)
    ...
    for i in todo_indices:
        ...
        try:
            ...
            types_str = ", ".join(result.get("waypoint_type", []))
